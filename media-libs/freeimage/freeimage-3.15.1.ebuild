...
src_prepare() {
		append-cflags -std=c99 -D_POSIX_SOURCE
		EPATCH_SOURCE="${WORKDIR}/${MY_PN}-${MY_PV}-patches" \
...
src_install() {
		...
		eend $?
		PKG_CONFIG_PATH="${D}/usr/$(get_libdir)/pkgconfig/" \
				pkg-config --exists ${PN} || \
