...
multilib_src_compile() {
	set_opts
	emake lib/libGLEW.so.${PV} "${myglewopts[@]}"
	mv lib/libGLEW.so.${SLOT}.* .
	emake -j1 clean lib NAME=GLEWmx
	emake lib/libGLEWmx.so.${PV} "${myglewopts[@]}" GLEW_MX=1 NAME=GLEWmx
	mv lib/libGLEWmx.so.${SLOT}.* .
}

multilib_src_install() {
	newlib.so libGLEW.so.${SLOT}.* libGLEW.so.${SLOT}
	newlib.so libGLEWmx.so.${SLOT}.* libGLEWmx.so.${SLOT}
}
